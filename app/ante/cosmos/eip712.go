...
// Deprecated: LegacyEip712SigVerificationDecorator Verify all signatures for a transaction and return an error if any are invalid. Note,
...
func (svd LegacyEip712SigVerificationDecorator) AnteHandle(ctx sdk.Context,
	...
	// no need to verify signatures on recheck transaction
	if ctx.IsReCheckTx() {
	...
	if !ok {
		return ctx, errorsmod.Wrapf(errortypes.ErrInvalidType, "transaction %T doesn't implement authsigning.SigVerifiableTx", tx)
	}

	authSignTx, ok := tx.(authsigning.Tx)
	if !ok {
		return ctx, errorsmod.Wrapf(errortypes.ErrInvalidType, "transaction %T doesn't implement the authsigning.Tx interface", tx)
	}
...
func VerifySignature(
	...
	case *signing.SingleSignatureData:
		...
		msgs := tx.GetMsgs()
		if len(msgs) == 0 {
			return errorsmod.Wrap(errortypes.ErrNoSignatures, "transaction doesn't contain any msgs to verify signature")
		}
		...
		if !ok {
			return errorsmod.Wrap(errortypes.ErrUnknownExtensionOptions, "transaction doesnt contain any extensions")
		}
		opts := txWithExtensions.GetExtensionOptions()
		if len(opts) != 1 {
			return errorsmod.Wrap(errortypes.ErrUnknownExtensionOptions, "transaction doesnt contain expected amount of extension options")
		}
		...
		if err != nil {
			return errorsmod.Wrap(err, "failed to create EIP-712 typed data from transaction")
		}
