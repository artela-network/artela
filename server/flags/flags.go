...
const (
	...
	EVMMaxTxGasWanted = "evm.max-transaction-gas-wanted"
)
...
// AddTxFlags adds common flags for commands to post transaction
func AddTxFlags(cmd *cobra.Command) (*cobra.Command, error) {
	...
	cmd.PersistentFlags().Float64(flags.FlagGasAdjustment, flags.DefaultGasAdjustment, "adjustment factor to be multiplied against the estimate returned by the transaction simulation; if the gas limit is set manually this flag is ignored ") //nolint:lll
