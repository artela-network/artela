...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 transaction.json --chain-id artelad_9000-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return errors.Wrap(err, "read transaction from file")
			}
			...
			if err != nil {
				return errors.Wrap(err, "wrap transaction to typed data")
			}
