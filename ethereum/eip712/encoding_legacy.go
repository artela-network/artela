...
func legacyDecodeAminoSignDoc(signDocBytes []byte) (apitypes.TypedData, error) {
	...
	if !ok {
		return apitypes.TypedData{}, errors.New("not a valid legacy message")
	}
...
func legacyValidatePayloadMessages(msgs []sdk.Msg) error {
	...
	for i, m := range msgs {
		...
		lm, ok := m.(sdk.LegacyMsg)
		if !ok {
			return errors.New("not a valid legacy message")
		}
		if len(lm.GetSigners()) != 1 {
			return errors.New("unable to build EIP-712 payload: expect exactly 1 signer")
		}

		if i == 0 {
			msgType = t
			msgSigner = lm.GetSigners()[0]
			continue
		}
		...
		if !msgSigner.Equals(lm.GetSigners()[0]) {
