...
// CosmosTxArgs contains the params to create a cosmos process
type CosmosTxArgs struct {
	// TxCfg is the client process config
	TxCfg client.TxConfig
	// Priv is the private key that will be used to sign the process
	Priv cryptotypes.PrivKey
	// ChainID is the chain's id on cosmos format, e.g. 'artela_9000-1'
	ChainID string
	// Gas to be used on the process
	Gas uint64
	// GasPrice to use on process
	GasPrice *sdkmath.Int
	// Fees is the fee to be used on the process (amount and denom)
	Fees sdk.Coins
	// FeeGranter is the account address of the fee granter
	FeeGranter sdk.AccAddress
	// Msgs slice of messages to include on the process
	Msgs []sdk.Msg
}

// PrepareCosmosTx creates a cosmos process and signs it with the provided messages and private key.
...
// signCosmosTx signs the cosmos process on the txBuilder provided using
...
// holds no valid process information.
//
